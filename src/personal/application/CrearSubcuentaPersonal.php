...
final class CrearSubcuentaPersonal
{
    ...
    public function ejecutar(array $datos): Cuenta
    {
        ...
        if (!CatalogoMaestroPersonal::existe($maestro)) {
            throw new InvalidArgumentException(_("El código maestro no existe en el plan principal"));
        }
        $padre = $this->cuentas->buscar($ctx->centroId, $ctx->personaId, 'X', $maestro);
        if ($padre === null || $padre->id === null) {
            throw new InvalidArgumentException(_("No hay cuenta padre de ese código maestro"));
        }
        $slug = strtolower(trim((string) ($datos['codigo'] ?? '')));
        $slug = preg_replace('/[^a-z0-9]/', '', $slug) ?? '';
        if ($slug === '' || strlen($slug) > 20) {
            throw new InvalidArgumentException(_("La etiqueta de la subcuenta debe ser alfanumérica (máx. 20)"));
        }
        $codigo = $maestro . '.' . $slug;
        if ($this->cuentas->buscar($ctx->centroId, $ctx->personaId, 'X', $codigo) !== null) {
            throw new InvalidArgumentException(_("Esa subcuenta ya existe"));
        }
