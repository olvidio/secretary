...
final class RecibirFicheroCopiaPersonal
{
    ...
    public static function obligatorio(Request $request, string $campo = 'dump'): string
    {
        $info = $request->file($campo);
        if ($info === null) {
            throw new InvalidArgumentException('Falta el fichero de copia personal');
        }
        $error = (int) ($info['error'] ?? UPLOAD_ERR_NO_FILE);
        if ($error === UPLOAD_ERR_INI_SIZE || $error === UPLOAD_ERR_FORM_SIZE) {
            throw new InvalidArgumentException('La copia supera el tamaño máximo (32 MB)');
        }
        ...
        if (strtolower(pathinfo($nombre, PATHINFO_EXTENSION)) !== 'json') {
            throw new InvalidArgumentException('El fichero debe ser una copia .json');
        }
        $size = (int) ($info['size'] ?? 0);
        if ($size <= 0 || $size > self::MAX_BYTES) {
            throw new InvalidArgumentException('La copia está vacía o supera 32 MB');
        }
