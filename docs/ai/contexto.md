...
- Saldos de caja/banco: `SUM(debe − haber)` de cuentas de tesorería (`CAJA` / `BANCO`). Saldos personales: cuentas `CC.*` del libro P. El disponible operativo de labores 7 vive aparte (`docs/dev/disponible.md`): tabla `saldos_disponibles` y aparcamiento `DISP.*`.
...
- Entrada G: un gasto con iniciales crea P/111, P/21, G/11 y el gasto (`ContrapartidasGastoGeneral`). P/212 es vivienda personal (sin G/11).
